...
from free_range_zoo.envs._base.logging_handlers import CSVLogger, SQLLogger
...
class BatchedAECEnv(ABC, AECEnv):
    ...
    def step(self, actions: torch.Tensor) -> None:
        ...
        # Extract action for the current agent if actions is a dict
        ...
        if isinstance(agent_action, torch.Tensor):
            self.actions[agent] = agent_action.to(device=self.device, dtype=self.metadata['action_dtype'])
        else:
            self.actions[agent] = torch.tensor(agent_action, device=self.device, dtype=self.metadata['action_dtype'])
