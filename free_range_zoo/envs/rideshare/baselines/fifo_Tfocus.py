"""
FIFO passenger selection baseline agent for the rideshare environment.

This module provides a baseline agent that always selects the first-arrived passenger for each agent in every
parallel environment. If multiple passengers share the earliest arrival, one is selected at random. The agent
...
class FirstInFirstOutTfocusBaseline(Agent):
    """
    Always select the first arrived passenger in the rideshare environment.

    This baseline agent always chooses the passenger that arrived first (FIFO) for each agent in every parallel
    ...
    def observe(self, observation: torch.Tensor) -> None:
        ...
        for batch in range(self.parallel_envs):
            ...
            if torch.any(accepted[batch]):
                argmin_store[batch][~accepted[batch, :self.t_mapping[batch].shape[0]]] = float('inf')

            if torch.any(riding[batch]):
                argmin_store[batch][~riding[batch, :self.t_mapping[batch].shape[0]]] = float('inf')
