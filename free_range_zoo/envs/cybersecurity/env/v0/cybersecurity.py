"""Cybersecurity Network Defense Environment (v0 - Original Implementation).

This module provides the cybersecurity environment where agents (attackers and
defenders) interact in a network environment. Attackers aim to exploit nodes
while defenders work to patch and protect the system.

This environment simulates a partially observable network where:
- Attackers try to increase the exploited state of nodes
- Defenders patch nodes to reduce exploitation
- Both agent types must operate with limited observation
...
from free_range_zoo.envs._base.v0.conversions import batched_aec_to_batched_parallel
...
from free_range_zoo.envs.cybersecurity.env.v0.spaces import actions, observations
from free_range_zoo.envs.cybersecurity.env.v0.structures.state import CybersecurityState
