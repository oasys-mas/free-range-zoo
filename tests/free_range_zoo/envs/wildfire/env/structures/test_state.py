...
class TestWildfireState(unittest.TestCase):
    def test_state_includes_to(self) -> None:
        self.assertTrue(hasattr(WildfireState, 'to'),
                        'WildfireState does not include to method')
        self.assertTrue(callable(WildfireState.to),
                        'WildfireState to method is not callable')

    def test_state_includes_save_initial(self) -> None:
        self.assertTrue(hasattr(WildfireState, 'save_initial'),
                        'WildfireState does not include save_initial method')
        self.assertTrue(callable(WildfireState.save_initial),
                        'WildfireState save_initial method is not callable')

    def test_state_includes_restore_initial(self) -> None:
        self.assertTrue(hasattr(WildfireState, 'restore_initial'),
                        'WildfireState does not include restore_initial method')
        self.assertTrue(callable(WildfireState.restore_initial),
                        'WildfireState restore_initial method is not callable')

    def test_state_includes_save_checkpoint(self) -> None:
        self.assertTrue(hasattr(WildfireState, 'save_checkpoint'),
                        'WildfireState does not include save_checkpoint method')
        self.assertTrue(callable(WildfireState.save_checkpoint),
                        'WildfireState save_checkpoint method is not callable')
