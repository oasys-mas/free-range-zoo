...
from free_range_zoo.free_range_zoo.envs.wildfire.env.transitions.suppressant_refill import SuppressantRefillTransition


class TestFireSpreads(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.state = WildfireState(
            fires=torch.zeros((self.parallel_envs, self.max_y, self.max_x), dtype=torch.int32, device=self.device) * -1,
            ...
            suppressants=torch.zeros((self.parallel_envs, self.num_agents), dtype=torch.float32, device=self.device),
        ...
        self.suppressant_refill_transition = SuppressantRefillTransition(
            ...
            equipment_bonuses=torch.tensor([0, 0, 0], dtype=torch.float32, device=self.device)
        ).to(self.device)
    ...
    def test_stochastic_refill(self):
        ...
        self.assertTrue(torch.allclose(result.suppressants, expected), f"""
