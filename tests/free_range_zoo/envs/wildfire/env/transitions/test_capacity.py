...
from free_range_zoo.free_range_zoo.envs.wildfire.env.transitions.capacity import CapacityTransition
...
class TestTransitionForward(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.capacity_transition = CapacityTransition(
            ...
            capacity_probabilities=torch.tensor([0.2, 0.5, 0.3], dtype=torch.float32)).to(self.device)
    ...
    def test_stochastic_tank_switch(self) -> None:
        ...
        self.assertTrue(torch.allclose(result_state.capacity, expected_capacities), f"""
            \rCapacity should match expected
                \rExpected:\n{expected_capacities}
                \rResult:\n{result_state.capacity}""")
        self.assertTrue(torch.allclose(result_state.suppressants, expected_suppressants), f"""
    ...
    def test_no_targets(self) -> None:
        ...
        for key in state_before.__annotations__:
            self.assertTrue(torch.allclose(getattr(state_before, key), getattr(state_after, key)), f"""
    ...
    def test_bonuses_from_refill_are_maintained_after_suppressant_increase(self) -> None:
        ...
        self.assertTrue(torch.allclose(expected_bonuses, received_bonuses), f"""
    ...
    def test_cpu_gpu_compatibility(self) -> None:
        ...
        for key in cpu_result.__annotations__:
            self.assertTrue(
                torch.allclose(getattr(cpu_result, key), getattr(gpu_result, key).cpu()), f"""
